...
void PrintEmployeeHeader() {
    cout << left
        << setw(8) << "ID"
        << setw(30) << "ФИО"
        << right << setw(10) << "Возраст"
        << "  "
        << left << setw(20) << "Должность"
        << right << setw(12) << "Зарплата"
        << endl;
...
void Add_Employee() {
    try {
        ...
        if (result->next()) {
            cout << "Сотрудник уже существует. Введите Enter для продолжения...\n"<<name_s<<"   "<<id_s; cin.get(); system("cls");
        ...
        cout << "Сотрудник добавлен. Введите Enter для продолжения...\n";
        cin.get(); system("cls");
